...
next_result:
	...
	if (r == LDAP_RES_SEARCH_RESULT)
	{
		if (l->pagesize > 0)
		{
			...
			if (!page_control)
			{
				l->ldap->error = LDAP_CONTROL_NOT_FOUND;
				if (controls)
					ldap_controls_free(controls);
				ldap_msgfree(result);
				return;
...
void LDAP_class_search(HalonHSLContext* hhc, HalonHSLArguments* args, HalonHSLValue* ret)
{
	...
	if (opts)
	{
		HalonHSLValue *k, *v;
		size_t z = 0;
		while ((v = HalonMTA_hsl_value_array_get(opts, z++, &k)))
		{
			char* opt;
			size_t optlen;
...
void LDAP_class_getpeerx509(HalonHSLContext* hhc, HalonHSLArguments* args, HalonHSLValue* ret)
{
	...
	HalonMTA_hsl_value_set(ret, HALONMTA_HSL_TYPE_X509, cert, 0);
}
...
bool MyLDAPResult::search(struct berval* cookie)
{
	...
	if (pagesize > 0)
	{
		int r = ldap_create_page_control(ldap->ld, pagesize, cookie, 1, &page_control);
		if (r != LDAP_SUCCESS)
		{
			ldap->error = r;
			return false;
